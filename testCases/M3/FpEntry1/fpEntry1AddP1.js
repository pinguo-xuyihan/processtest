...
*    时间(即时投放) ->优先级->文案显示->默认文案->icon显示->上传
...
casper.test.begin('v8banner process test', 5, function suite(test) {
  ...
  casper.then(function() {
    ...
    test.assertExists('.menu li:nth-of-type(5) ol.onChoos','已进入1号位');
    
    //-------------设置标题名------------·---
    this.sendKeys('input[data-key="title"]', 'fpEntry1AddP1', { reset: true});
  ...
  casper.then(function(){
      ...
      this.capture(config.captureRoot + 'fpEntry1AddP1.png');
  ...
  casper.run(function() {
    test.done();
      test.renderResults(true, 0, 'fpEntry1AddP1.xml');
  });
});
