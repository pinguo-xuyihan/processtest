/**
*    P8的测试路径为: 选择平台(IOS)->填写标题->选择投放语言(中文)->语言面板出现->
*   选择配置类型(运营投放)->选择模板类型(模板0)->选择投放策略(运营01轮 
*   播)->选择客户端版本(区间版本投放)-> 发布时间(即时投放) ->优先级->数据类
*   型(视频)->上传视频->启动单独配置->发布测试
*
**/
...
casper.test.begin('v8banner process test', 5, function suite(test) {
  ...
  casper.then(function() {
      ...
      //-------------设置标题名------------·---
      this.sendKeys('input[data-key="title"]', 'casperTestP6', { reset: true});
  ...
  casper.then(function(){
      ...
      this.click('.tab-panel li.item-li:nth-of-type(3) .multiselect-container li:nth-of-type(3) label');

      ///上传图片
    
      casper.page.uploadFile('#video', './test.mp4');
      
      this.waitForSelector('div[data-key="adVideo"] video');
  
  });

  casper.then(function () {

      test.assertExists('div[data-key="adVideo"] video', '视频已上传');
  ...
  casper.then(function(){
      ...
      this.capture(config.captureRoot + 'feedAddP3.png');
  ...
  casper.run(function() {
    test.done();
      test.renderResults(true, 0, 'feedAddP3.xml');
  });
});
