/**
*    P7的测试路径为: 选择平台(IOS)->填写标题->选择投放语言(中文)->语言面板出现->
*    选择配置类型(运营投放)->选择模板类型(模板0)->选择投放策略(运营01轮
*    播)->选择客户端版本(区间版本投放)-> 发布时间(即时投放)->发布账号主题->
*    选择投放标示->@对象填写->优先级->数据类型(多图)->上传图片->图片跳转链
*    接 (隐藏app)->发布测试
**/
...
casper.test.begin('v8banner process test', 5, function suite(test) {
  ...
  casper.then(function() {
      ...
      //-------------设置标题名------------·---
      this.sendKeys('input[data-key="title"]', 'feedAddP2', { reset: true});
  ...
  casper.then(function(){
      ...
      this.click('.tab-panel li.item-li:nth-of-type(3) .multiselect-container li:nth-of-type(2) label');

      casper.page.uploadFile('#multi', './100_100.png');
  ...
  casper.then(function(){
      ...
      this.capture(config.captureRoot + 'feedAddP2.png');
  ...
  casper.run(function() {
    test.done();
      test.renderResults(true, 0, 'feedAddP2.xml');
  });
});
